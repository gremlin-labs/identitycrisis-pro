# IdentityCrisis Pro - Image Generation Styles Specification

## Implementation Overview

This document provides detailed specifications for implementing 30 unique profile image generation styles in IdentityCrisis Pro. Each style includes specific prompt engineering instructions, technical parameters, and post-processing guidelines.

## Style Implementation Structure

Each style should be implemented with the following properties:
...
```javascript
{
  ...
  promptTemplate: "Base prompt with {placeholders}",
  promptSuffix: "Additional style-specific instructions",
...
### 2. UI Implementation
- Present styles in a grid or carousel format
- Show preview thumbnails for each style
- Group by category with collapsible sections
- Include search/filter functionality
- Add favorites/recent styles feature

### 3. Prompt Construction
```javascript
function constructPrompt(style, personDescription) {
...
```javascript
async function applyPostProcessing(image, style) {
  let processedImage = image;
  
  // Apply filters in order
  for (const filter of style.postProcessing.filters) {
    processedImage = await applyFilter(processedImage, filter);
  }
  
  // Apply adjustments
  processedImage = await applyAdjustments(processedImage, style.postProcessing.adjustments);
...
const qualityPresets = {
  "standard": { width: 512, height: 512, samples: 1 },
  "high": { width: 768, height: 768, samples: 2 },
  "ultra": { width: 1024, height: 1024, samples: 4 }
};
```

### 6. Error Handling
- Provide fallback styles if generation fails
- Implement retry logic with adjusted parameters
- Log style usage for analytics and improvement
- Handle API rate limits gracefully

### 7. Style Recommendations
```javascript
function recommendStyles(userProfile) {
  // Analyze user's previous choices
  // Consider identity purpose (social media platform, etc.)
  // Return top 5 recommended styles
}
```

---

This specification provides everything needed to implement the 30 unique image generation styles in IdentityCrisis Pro. Each style is crafted to serve specific identity needs while maintaining the playful, spy-themed brand voice of the application.